...
class PipelinePlanner:
    ...
    def evaluatePipeline(self, point, button):
        if button == Qt.LeftButton:
            ...
            # set the row count to zero each time
            self.dlg.tblImpacts.setRowCount(0)

            # get a reference to the Raptor Buffer layer - no checking taking place to check that it is present!
            lyrRaptor = QgsProject.instance().mapLayersByName("Raptor Buffer")[0]
            # get the geometry of all of the raptor buffers that are within the pipeline bounding box
            ...
            # get a reference to the BAEA Buffer layer - no checking taking place to check that it is present!
            ...
            for ba in baeas:
                valConstraint = "BAEA"
                valID = ba.attribute("nest_id")
                valStatus = ba.attribute("status")
                valDistance = pipeline.distance(ba.geometry().centroid())
                if ba.geometry().intersects(pipeline):
                    row = self.dlg.tblImpacts.rowCount()
                    self.dlg.tblImpacts.insertRow(row)
                    self.dlg.tblImpacts.setItem(row, 0, QTableWidgetItem(valConstraint))
                    self.dlg.tblImpacts.setItem(row, 1, QTableWidgetItem(str(valID)))
            ...
            # get a reference to the BUOWL Buffer layer - no checking taking place to check that it is present!
            lyrBUOWL = QgsProject.instance().mapLayersByName("BUOWL Buffer")[0]
            # get the geometry of all of the raptor buffers that are within the pipeline bounding box
            buowl = lyrBUOWL.getFeatures(pipeline.boundingBox())

            for owl in buowl:
                valConstraint = "BUOWL"
                valID = owl.attribute("habitat_id")
                valStatus = owl.attribute("recentstat")
                # runs an inverse buffer on the owl buffer geometry so it matches the original BUOWL layer
                valDistance = pipeline.distance(owl.geometry().buffer(-0.002, 5))
            ...
            # reset the pipeline rubberband so that it is clear for the next time
